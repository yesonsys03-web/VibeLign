...
def _render_markdown(data: ExplainData) -> str:
    ...
    if files:
        lines.extend(_render_reference_files(files))
    else:
        lines.append("- 나열할 파일이 없습니다.")
    return "\n".join(lines) + "\n"


def _render_reference_files(files: list[FileSummary]) -> list[str]:
    lines = ["파일 종류 요약:"]
...
def _file_status_with_time(item: FileSummary) -> str:
    status = _status_label(item.get("status", ""))
    modified_at = item.get("modified_at", "")
    return f"{status}, {modified_at}" if modified_at else status


def _file_kind_summary(files: list[FileSummary]) -> list[str]:
    counts: dict[str, int] = {}
    for item in files:
...
def _file_kind_label(item: FileSummary) -> str:
    kind = item.get("kind", "")
    path = item.get("path", "")
    if kind == "docs":
        return "문서"
