...
## 최종
- [ ] `vibeguard guard --strict` 실행 → 이상 없음 확인
- [ ] 이상 있으면: `vibeguard undo` 로 되돌리기
- [ ] 이상 없으면: `vibeguard checkpoint "완료: 작업 설명"`
"""

_ANTIGRAVITY_SETUP = """\
# VibeGuard + Antigravity 작업 흐름
...
**6단계 — vibeguard로 최종 검증**
```
vibeguard explain --write-report
vibeguard guard --strict
```

**문제 없으면** → `vibeguard checkpoint "완료"`
...
def run_export(args):
    ...
    export_root = root / "vibeguard_exports" / args.tool
