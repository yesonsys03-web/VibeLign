...
이 문서가 정의하는 것은 (a) 멀티 페르소나 기획 엔진 자체와 (b) 엔진의 두 진입점 — GUI 입력 바·CLI 명령 — 이다. 구현 순서는 통합기획안 §11 우선순위와 §12 PR 1~8을 따른다:
...
PR 4: 첫 페르소나 (agy --print) 실제 응답 — 본인 v1 dogfood 시작
PR 5: claude + codex 추가 + 3 페르소나 + mention 라우팅
```
...
사용자는 각 공식 CLI에서 직접 로그인한다.

```text
claude login
codex login 또는 codex 첫 실행 로그인
antigravity login 또는 antigravity 첫 실행 로그인
```

VibeLign은 로그인 토큰, 쿠키, 세션 파일을 직접 읽지 않는다. 공식 CLI를 subprocess로 실행하고 stdout/stderr 결과만 수집한다.
...
**작업 단위 매핑**: 아래 Task 1~9는 historical로, 통합기획안 §12 PR 1~8에 다음과 같이 분산된다. 한 PR에 한 Task만 들어가지는 않고, 각 PR은 여러 Task의 일부분만 가져간다.

| Task | 통합기획안 PR | 비고 |
|---|---|---|
| Task 1 (CLI 표면 추가 `vib plan`) | PR 3 | 사용자 표면이 아니라 엔진 검증 진입점 |
| Task 2 (Markdown writer + 저장소) | PR 3 | 엔진 골격 |
| Task 3 (CLI adapter) | PR 4 (agy 1개) + PR 5 (claude, codex) | 분할 |
| Task 4 (멀티 AI orchestration) | PR 5 | 3 페르소나 best-effort |
| Task 5 (초보자 질문 흐름) | PR 3 | 엔진 입력 |
| Task 6 (CLI 통합 테스트) | 각 PR마다 분산 | 단일 Task로 끝나지 않음 |
| Task 7 (문서와 manual 갱신) | PR 6 | patch 퇴출과 함께 |
| Task 8 (기획방 채팅 UI) | PR 1 + PR 3 + PR 5 | 입력 바·기획방·mention 라우팅으로 분할 |
| Task 9 (페르소나 캐릭터 커스터마이즈) | PR 8 | 마지막 |

**1차 우선 작업은 Task 8 (GUI 입력 바·기획방) → Task 2/5 (엔진 골격) → Task 1 (CLI 진입점) → Task 3/4 (LLM 호출)** 순이다. Task 1을 가장 먼저 만들지 않는다.
...
가장 큰 기술 리스크는 sprite-gen이 아니라 **구독 CLI 자동화 안정성**이다.
...
이를 한 번에 구현하면 실패할 가능성이 높다. 보수적 구현 순서는 통합기획안 §11 우선순위와 §12 PR 1~8에 정렬한다:
...
4. 첫 페르소나 `agy --print` 실제 응답 — 본인 v1 dogfood 시작 (PR 4)
5. `claude` + `codex` 추가 + 3 페르소나 best-effort + mention 라우팅 + 정적 아바타 (PR 5)
6. patch 퇴출 + 초보 문서 정리 (PR 6)
7. 자동 watch/anchor/guard 정밀화 + 에러 인간화 (PR 7)
8. 페르소나 preset 커스터마이즈 (PR 8)
9. sprite-gen 고급 생성/큐레이션 (별도 PR 보류)
