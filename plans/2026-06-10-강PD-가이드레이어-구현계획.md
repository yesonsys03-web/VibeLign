...
폴백 분기 제거(품질 리뷰): 가이드 ON/OFF는 전역 설정(GUIDE_ENABLED_KEY)이라 '프로젝트 먼저 선택' 전제가 성립하지 않고, useGuide가 항상 boolean을 주므로 죽은 코드 — props도 필수로:
```tsx
        {/* 초보자 가이드 토글 섹션 */}
...
- 축하 오발화 희박 경로(구현 품질 리뷰 기록): `listChangedFiles` 일시 예외로 changedFileCount가 null이 되면 6️⃣→4️⃣ 추론 전환이 생겨 축하가 오발화할 수 있음 — 프로젝트당 최대 1회·발생 희박이라 v1 수용. 손볼 일이 생기면 "next=4가 count===0에서 온 경우만 발화"로 좁히는 선택지
