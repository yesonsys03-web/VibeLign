...
| Antigravity | 미나 | `agy` headless/print 계열 | 본인 dogfood 욕구는 가장 크지만 약관 회색지대와 headless 계약 미확정 리스크가 가장 크다. |

결론: **PR 4 시작 시 실제 로컬 probe를 돌려 Codex를 기본 1순위로 두되, agy headless가 안정적으로 확인되고 사용자가 dogfood 우선 결정을 유지하면 미나/agy로 시작할 수 있다.** 어떤 경우에도 PR 4는 한 페르소나만 연결한다.
...
- README/README.ko
  - 이미 추가된 "Third-Party AI CLI 고지"가 실제 LLM relay까지 커버하는지 확인하고 필요 시 한 줄 보강(중복 추가 금지)
...
6. 실제 로컬 probe는 PR 작성자가 접근 가능한 OS에서 수행한다. macOS에서 시작해도 되지만, Windows는 fake runner/단위 테스트로 subprocess 계약을 고정하고 release 전 수동 QA 또는 CI 환경에서 별도 확인한다. 특히 agy 헤드리스 플래그/동작은 OS별로 다를 수 있고(Windows는 WSL 경유 가능성 포함), 한 OS에서 확인한 결과를 모든 OS 보장처럼 문서화하지 않는다.
...
```bash
agy <headless-or-print-flag> "<prompt>"
```

`agy` 플래그명은 구현 전에 로컬 help/probe로 확정하고, 확정 전에는 문서와 코드에 가정값을 박지 않는다.
...
```text
Antigravity 연결은 공식 CLI를 그대로 실행하지만, 제3자 앱에서 쓰는 방식은 벤더 약관상 회색지대일 수 있어요.
```
...
- README/README.ko의 Third-Party AI CLI 고지가 LLM relay를 커버한다(이미 추가됨; 필요 시 보강만, 중복 추가 금지).
