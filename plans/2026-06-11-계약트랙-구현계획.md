...
**Architecture:** 추출은 readiness 판정(`judge_readiness`)과 같은 패턴 — `save_planning_chat_as_markdown`에서 `run_active_ai` 1회 호출, strict JSON 파싱, 실패 시 "확인 못 함" 폴백(퇴행 0). 저장은 cards.json 관례(`contract.json` = 소스, Markdown은 파생 뷰). 프론트는 응답의 `contract` 필드를 받아 ② 지시문 주입(`buildPlanningWorkInstruction`)과 ③ 범위 비교 리포트(`scopeReport` 순수 함수, Home이 guard 실행 시점에 `listChangedFiles` 직접 조회)에 사용한다.
...
/// 확정 시점 1회 추출. AI 없음·실패·파싱 실패 = None (readiness '확인 못 함' 패턴 — 퇴행 0).
...
```rust
/// 작업 계약 섹션 — 읽기 전용 노출(spec §1 Q3). 추출 실패는 readiness와 같은 솔직한 한 줄.
pub(crate) fn contract_section(contract: Option<&PlanningContract>) -> String {
    let Some(contract) = contract else {
        return "## 작업 계약: 확인 못 함 (활성 AI 없음)\n\n".to_string();
    };
...
```rust
    ...
    fn contract_section_is_honest_when_missing_or_scopeless() {
        assert!(contract_section(None).contains("확인 못 함"));
...
git commit -m "feat(contract): 기획안에 '작업 계약' 읽기 전용 섹션 렌더 — 실패는 '확인 못 함' 정직 표기"
```
...
- [ ] **Step 2: save 커맨드에 추출 통합**

planning_chat.rs `save_planning_chat_as_markdown` — `let cards = read_cards(&session_dir);`(:252) 바로 아래에 추가:
```rust
        // 작업 계약 추출(spec §4) — readiness와 같은 확정 시점 1회. 실패 = None(퇴행 0).
...
describe("scopeReport", () => {
  ...
  it("도구 메타데이터 경로는 비교에서 제외한다 (.vibelign/.omc/.git)", () => {
    const report = scopeReport(SCOPE, [".vibelign/report.md", ".omc/state.json", ".git/HEAD"]);
  ...
  it("변경이 없으면 0/빈 목록 (비-git 프로젝트의 항상 상태 — spec §7-1)", () => {
    expect(scopeReport(SCOPE, [])).toEqual({ inScope: 0, outOfScope: [] });
  });
...
/**
 * scope가 비면 null(리포트 성립 불가 — 표시하지 않는다).
...
```ts
      ...
      if (projectDir && planningContract && planningContract.scope.length > 0) {
        try {
          ...
          setScopeReportResult(null); // 조회 실패 = 리포트 생략(추측 안내 금지)
        }
      } else {
...
- [ ] **AI 미설정 폴백(퇴행 0)**: AI CLI 없는 환경에서 저장 → Markdown에 "## 작업 계약: 확인 못 함" 한 줄, contract.json 미생성, 지시문은 기존 보일러플레이트 그대로, 홈 리포트 미노출 — **기존 저장 흐름 어떤 것도 깨지지 않음**
...
- [ ] **비-git 프로젝트**: 변경이 항상 0으로 읽혀 리포트가 "범위 안 0건" — 무해한지 육안 판단(거슬리면 후속에서 비-git 시 미노출 검토 메모)
...
- 계약 나이·stale 표시 / 비-git 프로젝트 리포트 미노출 검토
