...
> QA 피드백(2026-05-28): 우측 글자 배지만으로는 카테고리 색에 묻혀 눈에 띄지 않음 →
> **행 배경 색조 강조**로 변경.

- 파일(변경됨): **행 전체 배경**을 diff 스타일 색조로 강조 — 수정=주황 `rgba(245,158,11,0.24)`,
  신규=녹색 `rgba(34,197,94,0.22)`. 카테고리 tint 를 덮어쓰되, **선택 행은 다크 배경 유지**.
  추가로 우측(`marginLeft:auto`)에 `M`(진한 주황 `#92400e`)/`U`(진한 녹색 `#166534`) 글자 —
  색조 위에서도 또렷하게.
- 디렉토리: `changedCount > 0`이면 우측에 숫자 **pill 배지**(둥근 배경)로 가시성 확보.
- active(선택) 행은 글자/숫자 흰색으로 대비 유지.
