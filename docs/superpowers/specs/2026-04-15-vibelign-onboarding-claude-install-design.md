...
- WSL에서 `claude` 실행 가능
- 위 3개는 **설치 스크립트 종료 메시지와 무관하게**, 새 셸에서 실제 명령 실행으로만 판정한다.
...
- Windows **CMD / PowerShell / WSL** 에서 `claude --version` 성공, 또는 실패 원인이 구조화되어 다음 경로(CMD fallback / WSL / 수동 진행)로 안전하게 전환 가능
