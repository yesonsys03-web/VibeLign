...
`vibelign/action_engine/action_planner.py`의 `_classify_issue()`는 현재 문자열 파싱으로 `action_type`을 결정한다. 이슈에 `category` 필드가 추가되면, 이를 우선 참조하도록 변경한다.
