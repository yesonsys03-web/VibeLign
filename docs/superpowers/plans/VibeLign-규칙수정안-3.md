...
> **현재 코드 대조 메모 (2026-05-14):** 이 문서의 최종 기준은 계속 §12-5 / §12-6 이지만, 일부 구현 전제는 최신 코드와 다르다. 현재 `vibelign/core/git_hooks.py` 에는 pre-commit 뿐 아니라 post-commit 기록 블록도 존재한다. pre-commit 은 별도 줄수 검사 스크립트를 직접 실행하지 않고 `vib guard --strict` 를 호출하므로, 줄수 enforcement 는 guard 내부에 들어가야 한다. 반면 문서가 말하는 300/500 줄수 차단 + legacy baseline 동결은 아직 `vib_guard_cmd.py` 에 명확히 반영된 상태가 아니며, `risk_analyzer.py` 도 여전히 500/800/1000 단계 기준을 사용한다. 따라서 이 문서는 **구현 완료 문서가 아니라 Phase 1~2 착수 전 보정이 필요한 실행 계획**으로 읽는다.
