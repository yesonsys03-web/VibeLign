# VibeLign 온보딩 내 Claude Code 자동 설치·복구 기획안

*작성일: 2026-04-13 | 최종 수정: 2026-04-15 (Anthropic 공식 native install 스크립트 기준으로 재정렬)*
*목적: 별도 인스톨러를 제공하는 것이 아니라, VibeLign 첫 실행 온보딩 화면 안에서 Claude Code **공식 native install 스크립트**를 자동으로 시도하고, 실패 시 원인을 진단해 다음 성공 경로로 넘겨 주는 통합 온보딩 플로우를 설계한다.*

> **현재 구현 대조 메모 (2026-05-14):** 이 문서는 제품 비전과 v1 목표를 함께 담은 초안이다. 현재 GUI/Rust 구현은 설치 스크립트 실행, 새 셸 `claude --version` 검증, PATH 보정, WSL 설치 보조, uninstall 경로를 포함하지만, `claude doctor` 는 non-TTY/Ink raw-mode 한계로 성공 조건에서 제외되어 있고 PTY 기반 “첫 응답 토큰 수신” 검증도 장기 목표에 가깝다. 따라서 구현자는 아래 내용을 그대로 성공 조건으로 삼지 말고, **v1 현실 기준 = `claude --version` + 대표 셸 PATH + 로그인 안내**, **장기 기준 = PTY/doctor/대화 가능성 자동 검증**으로 분리해서 읽어야 한다.
...
> **설치 위치:** 공식 문서상 기본 경로는 `~/.local/bin/claude` (mac/linux/wsl), `%USERPROFILE%\.local\bin\claude.exe` (Windows) 이다. 단, Phase 0 Windows VM 에서는 실패한 설치가 `C:\Users\topye\claude` 0-byte artifact 를 남긴 사례가 있어 문서 경로만으로 성공을 판정하면 안 된다.
> **검증 명령:** `claude --version`, `claude doctor`.
> **로그인 명령:** `claude` 실행 시 브라우저 prompt. 별도 비상호작용 `claude auth status` 명령은 공식 setup 문서에 노출되어 있지 않다. 다만 Phase 0 macOS 실기기에서는 `claude auth status` 가 JSON 응답을 반환했으므로, v1 구현 시에는 “공식 문서에 노출된 명령”과 “실기기에서 관측된 명령”을 구분해서 다뤄야 한다.
...
### 비즈니스 임팩트
원클릭 설치 경험이 VibeLign의 **킬러 차별점**이 될 수 있음. Claude Code 자체는 개발자 도구라 진입이 어렵지만, VibeLign이 **첫 실행 온보딩 안에서 Claude Code 공식 설치를 우선 시도하고, 실패해도 원인을 진단해 다음 성공 경로로 이어 주는 친화적 진입 레이어**가 되면 시장이 완전히 달라짐.
...
- 온보딩 성공의 기준은 파일이 깔린 상태가 아니라, **Windows에서는 CMD / PowerShell / WSL, macOS에서는 bash / zsh에서 사용자가 `claude`를 실행할 수 있거나, 실패 시 다음 성공 경로로 즉시 넘어갈 수 있는 상태**다.
- 설치 스크립트의 `Installation complete` 문구나 종료 코드만으로는 성공 판정하지 않는다. 새 셸에서 실제 `claude` 실행 검증이 통과해야 한다.
...
### 옵션 A — 온보딩 화면 내 자동 설치 + 실패 복구 (1차 출시 목표)

VibeLign이 **첫 실행 온보딩 화면 안에서** Claude Code 자동 설치를 우선 시도하고, 설치가 끝나면 사용자는 평소처럼 터미널에서 `claude`를 실행한다. 자동 설치가 실패하면 VibeLign이 원인을 분류해 재시도, fallback, 수동 진행 중 다음 성공 경로를 즉시 제시한다.

**흐름:**
1. 사용자가 VibeLign을 실행하면 온보딩 화면 진입
2. 온보딩에서 자동 환경 점검 → Claude Code **공식 설치 경로** 선택 및 실행
3. 설치/검증 완료 → Windows는 CMD / PowerShell / WSL, macOS는 bash / zsh에서 `claude` 실행 가능하도록 자동 설정
4. 온보딩 안에서 로그인과 첫 실행까지 안내
5. 설치 완료 후 VibeLign이 다음 단계(예: 안전한 AI 작업 흐름, 설정, 가이드)로 연결

#### 온보딩 상태 모델 (제품 관점)

이 기능은 “한 번에 조용히 끝나는 설치 마법”처럼 보이더라도, 내부적으로는 상태가 분명해야 한다.

| 상태 | 사용자 의미 | 보여줄 버튼/행동 |
|---|---|---|
| `idle` | 시작 전 | 시작 |
| `diagnosing` | 내 환경을 확인 중 | 대기 |
| `needs_git` | Windows에서 Git for Windows가 먼저 필요함 | Git 설치 / 다시 확인 |
| `ready_to_install` | 자동 설치를 시작할 준비가 됨 | 자동 설치 시작 |
| `installing_native` | PowerShell 또는 CMD로 설치 중 | 대기 |
| `installing_wsl` | WSL 안에서 설치 중 | 대기 |
| `verifying_shells` | 설치가 끝난 뒤 실제 실행 가능 여부를 확인 중 | 대기 |
| `needs_cmd_fallback` | PowerShell 대신 CMD로 다시 시도해야 함 | CMD로 다시 시도 |
| `needs_wsl_fallback` | Windows native 대신 WSL 경로가 더 안전함 | WSL로 진행 |
| `needs_manual_step` | 자동으로 끝낼 수 없어 사용자가 한 단계 직접 해야 함 | 수동 단계 보기 |
| `login_required` | 설치는 됐고 로그인만 남음 | 로그인 시작 |
| `probing_login` | 실제로 대화 가능한지 마지막 확인 중 | 대기 |
| `success` | 이제 Claude Code를 쓸 수 있음 | Claude 실행 |
| `blocked` | 이 환경에서는 자동 진행이 멈춤 | 로그 공유 / 다른 경로 선택 |

제품 메시지는 항상 상태 기반이어야 한다. 즉, “에러가 났다”가 아니라 “지금은 `needs_git` 상태라서 Git 설치가 다음 행동이다”처럼 보여줘야 코알못도 길을 잃지 않는다.

#### 프론트/백엔드 데이터 계약 (제품 요약)

구현팀이 화면을 빠르게 붙일 수 있도록, 온보딩은 매 단계마다 **현재 상태 snapshot 1개**를 프론트로 넘기고, 장시간 작업 중에는 **progress event** 를 추가로 push 하는 구조로 고정한다.

- snapshot 에 반드시 들어갈 것
  - 현재 상태 (`idle`, `diagnosing`, `needs_git`, `installing_native`, `verifying_shells`, `success` 등)
  - 현재 OS / 설치 경로 종류 (`native-powershell`, `native-cmd`, `wsl`, `manual`)
  - 다음 행동 1개 (`install_git`, `retry_with_cmd`, `continue_with_wsl`, `open_manual_steps`, `start_login`, `launch_claude` 등)
  - 화면 headline / detail / primary button label
  - 진단 플래그 (`gitInstalled`, `wslAvailable`, `claudeOnPath`, `claudeVersionOk`, `claudeDoctorOk` 등)
  - 마지막 에러 코드 (`missing_git`, `installer_false_success`, `installer_oom`, `placeholder_artifact`, `command_not_found` 등)

- progress event 에 반드시 들어갈 것
  - 현재 phase (`diagnose`, `install`, `verify`, `login`)
  - 상태 이름
  - step id (`run_powershell_installer`, `run_cmd_installer`, `verify_version`, `verify_doctor`, `probe_login` 등)
  - 시작/진행/성공/실패 여부
  - 사용자에게 보여줄 메시지
  - 필요 시 로그 chunk / 관측된 경로 / 에러 코드

- 제품 원칙
  - 프론트는 가능한 한 문구를 즉석 조합하지 않고 snapshot 을 그대로 그린다.
  - 사용자가 지금 해야 할 행동은 항상 하나의 primary action 으로 보인다.
  - 로딩 중에도 현재 단계가 무엇인지 event 로 계속 보인다.
  - 로그 공유/지원 대응은 마지막 에러 코드와 로그 chunk 를 기준으로 한다.
...
즉, 초기 목표는 **VibeLign 내부 온보딩에서 자동 설치를 우선 시도하고, 실패해도 첫 실행 가능한 상태까지 사용자를 안내하는 것**이지, Claude Code용 별도 설치 전용 앱을 만드는 것이 아니다.
...
**온보딩 "성공"의 정의**: 제품 최종 목표는 `claude --version` 통과가 아니라, **로그인까지 완료되어 `claude`를 실행하면 대화가 시작되는 상태**다. 단, 2026-05-14 현재 구현 기준에서는 `claude doctor`/PTY 자동 대화 검증이 non-TTY·로그인 UX 제약을 받으므로, v1 판정은 “대표 셸에서 `claude --version` 통과 + PATH 보정 + 로그인 안내 진입”으로 둔다.

단, 제품 UX 관점에서는 로그인 이전에 자동 설치가 막히더라도 사용자가 다음 행동을 명확히 이해하고 진행할 수 있으면 실패 경험이 아니라 **가이드된 전환 상태**로 봐야 한다.
...
- **데이터 계약**: command 응답은 항상 최신 onboarding snapshot 1개를 반환하고, event 는 progress 보조 채널로만 사용한다. source of truth 는 snapshot 이다.
...
이 기획의 핵심은 “무조건 모든 환경을 한 번에 완벽 지원”이 아니라, **사용자가 실제로 가장 많이 여는 대표 셸에서는 첫 버전부터 `claude` 실행까지 최대한 자동으로 밀어주고, 막히면 다음 성공 경로로 곧바로 전환시키는 것**이다.
...
4. 설치 후 검증
   ...
   - install stdout/stderr 에 fatal error (`out of memory`, crash, placeholder artifact 생성 등)가 보이면 success footer 가 있어도 실패로 분류
   - `claude` 가 0-byte 파일/비실행 artifact 로 남으면 실패로 분류하고 CMD fallback / 더 큰 장비 재시도 / 로그 공유를 제안

#### Windows 상태 전이 요약

- 기본 흐름: `idle` → `diagnosing` → `ready_to_install` → `installing_native` → `verifying_shells` → `login_required` → `probing_login` → `success`
- Git 미설치: `diagnosing` → `needs_git`
- PowerShell 실패 후 CMD 후보 존재: `verifying_shells` 또는 install failure → `needs_cmd_fallback`
- native 경로보다 WSL이 안전: `verifying_shells` 또는 install failure → `needs_wsl_fallback`
- 자동 처리 한계: `needs_manual_step`
- unrecoverable crash / unsupported 환경: `blocked`
5. 로그인 검증
...
- Windows와 macOS 모두에서 사용자가 실제로 자주 여는 대표 셸에서 `claude`가 실행되어야 가장 이상적인 온보딩 완료라고 볼 수 있다.
...
- 단, 공식 설치 스크립트의 성공 문구 자체는 신뢰하지 않는다. Phase 0 Windows VM 에서 선행 조건 미충족과 Bun OOM 모두 success footer 를 출력한 사례가 있었기 때문이다.
