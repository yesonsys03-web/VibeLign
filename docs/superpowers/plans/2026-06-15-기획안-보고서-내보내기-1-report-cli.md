...
- IR 은 `source_plan_path` 를 CLI 증분 1부터 보존한다. HTML 렌더에는 쓰지 않더라도 후속 GUI/캐시/재생성 추적에서 필요하다.
- `--output` 은 프로젝트 상대경로만 허용하고, 기존 파일은 `--force` 없이는 덮어쓰지 않는다.
- `--json` 에러 계약을 지키기 위해 `--type` 은 argparse `choices` 로 막지 않고 커맨드 내부에서 검증한다.
...
- `vibelign/cli/vib_cli.py` — 최상위 help metavar 에 `report` 노출
...
- Modify: `vibelign/cli/vib_cli.py` (`report` 를 최상위 help metavar 에 추가)
...
`vibelign/cli/vib_cli.py` 의 `parser.add_subparsers(..., metavar=...)` 고정 문자열에도
`report` 를 추가한다. 실행 등록은 `cli_command_groups.py` 에서 끝나지만, 이 문자열을
갱신하지 않으면 `vib --help` 최상위 명령 목록에 `report` 가 보이지 않는다.
...
```bash
git add vibelign/commands/vib_report_cmd.py vibelign/cli/cli_command_groups.py vibelign/cli/vib_cli.py tests/cli/test_vib_report_cmd.py
...
- **리뷰 반영:** 실제 `vib plan` 섹션명(`만들고 싶은 이유`)을 `problem`으로 매핑, 번호 목록/일반 목록 줄 파싱, `--output` 프로젝트 상대경로 제한, explicit output 의 `--force` 없는 덮어쓰기 방지, 기본 출력 충돌 회피, 최상위 help 에 `report` 노출, `--json` 에러 계약을 위한 커맨드 내부 `--type` 검증, `source_plan_path` 보존을 테스트와 구현 지시문에 반영.
- **크로스플랫폼 가드:** `_relative_output_path` 의 `is_absolute()` 가 플랫폼 의존이라 Windows 에서 `/tmp/x` 가 드라이브 루트로 새던 문제를, POSIX·Windows 양쪽 규칙(절대성/드라이브/루트/`..`)으로 검사해 OS 무관하게 거부하도록 보강(테스트에 드라이브·UNC·드라이브루트 케이스 추가). 긴 한글 제목은 `_report_slug` 에서 해시 suffix 를 붙여 줄여 design §13.4 의 MAX_PATH 리스크를 CLI 증분 1부터 낮춘다.

## 후속 계획 (이 계획 완료 후)

- **계획 2:** GUI 연동 — `src/lib/vib/planning.ts` invoke 래퍼(`generatePlanningReport`) + Rust command(`vib report` sidecar 호출, `capture_with_stdin_timeout` 300s) + `PlanDocView` "📄 보고서로 내보내기" 버튼/모달(`GuardResultModal` 패턴) + 웹뷰 PDF 생성. **선결**: PDF 생성 경로는 design §13.1 a/b/c 중 확정 후 진입(Tauri v2 stable core 엔 무인 `printToPDF` 없음). **폰트**: 결정적 Win/Mac 출력용 OFL 한글 `@font-face` 번들 임베드를 이 증분에서 도입(증분 1 HTML 은 시스템 폰트 폴백, design §13.2).
