...
@pytest.mark.parametrize(
    "output", ["/tmp/report.html", "../report.html", "out/../report.html"]
)
...
def write_report(
    ...
    *,
    ...
    output 이 주어지면 프로젝트 상대 경로만 허용하고, 없으면
...
def run_vib_report(args: object) -> None:
    ...
    try:
        dest = write_report(
            root, model, html, slug_source=slug_source, output=raw.output
        )
    except ValueError as exc:
        _fail(want_json, str(exc))
        return
...
- **리뷰 반영:** 실제 `vib plan` 섹션명(`만들고 싶은 이유`)을 `problem`으로 매핑, 번호 목록/일반 목록 줄 파싱, `--output` 프로젝트 상대경로 제한, 기본 출력 충돌 회피, 최상위 help 에 `report` 노출을 테스트와 구현 지시문에 반영.
