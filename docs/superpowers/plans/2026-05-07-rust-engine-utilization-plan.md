...
- `cargo test --manifest-path "vibelign-core/Cargo.toml"` → 90 passed (2026-05-09 unicode parity 추가 후, 이전 74→89→90; Phase 0 lib expose / Phase 3 PoC 후에도 90 유지).
- `cargo build --manifest-path "vibelign-gui/src-tauri/Cargo.toml"` → 0 errors, 86 crates compiled (2026-05-09 vibelign-core path 의존 추가 후 PoC 빌드 검증, consumer #2 추가 후 incremental 0 errors).
- `npx tsc --noEmit` (vibelign-gui) → No errors found (2026-05-09 `checkpointList` / `undoCheckpoint` direct bridge 마이그레이션 후).
