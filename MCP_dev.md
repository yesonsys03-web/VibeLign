...
| 커맨드 | 이유 |
|--------|------|
| `vib start` | 사람이 세션 시작 시 1회 실행하는 bootstrap 명령 — MCP 도구 노출보다 Claude Code/Cursor 자동 등록이 적합 |
...
### Phase 4 — Claude Code / Cursor 연동

- [x] `vib start`가 자동으로 `.claude/settings.json`에 MCP 등록 (`_register_mcp_claude`)
- [x] `vib start`가 자동으로 `.cursor/mcp.json`에 MCP 등록 (`_register_mcp_cursor`)
