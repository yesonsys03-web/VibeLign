...
- [ ] `vibelign/mcp_server.py` 파일 생성 (MCP 서버 진입점)
- [ ] `pyproject.toml`에 `vibelign-mcp` 스크립트 엔트리포인트 추가

### Phase 2 — 핵심 도구 구현

- [ ] `checkpoint_create` 도구 구현
- [ ] `checkpoint_list` 도구 구현
- [ ] `checkpoint_restore` 도구 구현
- [ ] `project_context_get` 도구 구현
...
- [ ] `.claude/settings.json`에 MCP 서버 등록 방법 문서화
- [ ] `vib init`이 자동으로 MCP 등록하도록 업데이트
