...
mod tests {
    ...
    fn parses_success_json_from_stdout() {
        let response = parse_plan_stdout(
            r##"{"ok":true,"output_path":"plans/app.md","absolute_output_path":"/tmp/app/plans/app.md","markdown":"# App","fallback_reason":"template_only","session_id":"plan_1"}"##,
        )
        .expect("parse");

        assert!(response.ok);
        assert_eq!(response.output_path.as_deref(), Some("plans/app.md"));
        assert_eq!(response.fallback_reason.as_deref(), Some("template_only"));
    }
