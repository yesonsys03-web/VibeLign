...
> **사후 정정(2026-06-14)**: 위 §1의 "JS/앱 전체가 진짜 멈추진 않는다"는 **틀린 전제였다.** 실제로는 `generate_design_mockup`/`synthesize_style`가 **동기 `#[tauri::command] fn`**이라 ~1–2분 CLI 서브프로세스가 **Tauri 메인 스레드를 블로킹** → 웹뷰 전체가 얼어 탭 이동도 막혔다(사용자 첫 보고 "앱이 멈춘 것 같은"이 문자 그대로였음). 프론트 상태 외부화(아래)는 *결과 보존*에 필요하지만 freeze 자체는 못 고친다. 별도로 두 커맨드를 `async fn` + `tauri::async_runtime::spawn_blocking`으로 전환해 메인 스레드를 보존한다(코드 전반의 기존 패턴 — `commands/code.rs` 주석 참조). 두 수정이 합쳐져야 "백그라운드 생성 + 앱 사용 가능"이 성립.

**비목표**
- Rust 이벤트 스트리밍 **실시간 진행률 %**(과설계). 단, 메인 스레드 블로킹 해소를 위한 `async`+`spawn_blocking` 전환은 **목표에 포함**(위 정정).
