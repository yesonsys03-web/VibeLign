...
export default function CodeExplorerPlanningContext({ prompt, outputPath, contract, docStale = false }: CodeExplorerPlanningContextProps) {
  const [collapsed, setCollapsed] = useState(planCardCollapsed);
  const toggle = () =>
    setCollapsed((v) => {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", gap: 10, alignItems: "center", minWidth: 0, flexWrap: "wrap" }}>
        <div style={{ flex: 1, display: "flex", gap: 10, alignItems: "center", minWidth: 220 }}>
          <button
            ...
            title={collapsed ? "기획안 카드 펼치기" : "기획안 카드 접기"}
      ...
      {!collapsed && (
        <>
          <div style={{ fontSize: 12, color: "#666", fontWeight: 700, overflowWrap: "anywhere" }}>{outputPath}</div>
          {docStale && (
            <div style={{ fontSize: 12, color: "#B45309", background: "#FEF3C7", border: "1px solid #FDE68A", borderRadius: 4, padding: "6px 8px", fontWeight: 700 }}>
