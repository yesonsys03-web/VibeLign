    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <div className="page-header" style={{ padding: "12px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button className="btn btn-ghost btn-sm" onClick={onBack} style={{ fontSize: 11 }} type="button">← 목록</button>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: "#666" }}>{command.usage}</div>
          </div>
        </div>
      </div>
          ...
          <div className="card" style={{ marginTop: 12, padding: "12px 14px" }}>
            ...
            <div style={{ display: "grid", gap: 6 }}>
              {command.flags.map((flag, index) => {
                if (flag.type === "select") {
                  return (
                    <div key={`${flag.label}:${index}`} style={{ display: "grid", gap: 4 }}>
                      <div style={{ fontSize: 10, fontWeight: 900, color: "#555" }}>{flag.label}</div>
                ...
                return (
                  <div key={`${flag.label}:${index}`} style={{ display: "flex", gap: 6, alignItems: "baseline", fontSize: 11, lineHeight: 1.45 }}>
    ...
    <div>
      {guide.map((step, index) => (
        <div key={`${step.step}:${index}`} className="card" style={{ marginBottom: 8, padding: "12px 14px" }}>
          ...
          {step.subtitle ? <div style={{ fontSize: 10, color: "#888", marginBottom: 8, marginLeft: 2 }}>{step.subtitle}</div> : null}
          {step.lines.map((line, lineIndex) => {
            ...
            if (line.t === "label") {
              return <div key={`${line.t}:${lineIndex}`} style={{ fontSize: 10, fontWeight: 800, color: "#888", marginTop: 6, marginBottom: 2 }}>{line.v}</div>;
            }
            if (line.t === "error") {
            ...
            return <div key={`${line.t}:${lineIndex}`} style={{ fontSize: 11, color: "#444", lineHeight: 1.6, marginBottom: 2 }}>{line.v}</div>;
          })}
          {step.warn ? (
            <div style={{ marginTop: 8, fontSize: 10, fontWeight: 700, color: "#FFD166", background: "#FFD16618", border: "1.5px solid #FFD16666", padding: "5px 10px" }}>⚠ {step.warn}</div>
          ) : null}
        </div>
      ))}
    </div>
