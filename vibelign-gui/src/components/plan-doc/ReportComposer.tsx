...
// === ANCHOR: REPORTCOMPOSER_COUNTPOLISHABLEBLOCKS_START ===
...
// === ANCHOR: REPORTCOMPOSER_COUNTPOLISHABLEBLOCKS_END ===
...
export function ReportComposer({ planPath, cwd, layout, onClose, onReviewRequest, defaultType }: ReportComposerProps) {
  ...
  // === ANCHOR: REPORTCOMPOSER_HANDLEGENERATE_START ===
  ...
  // === ANCHOR: REPORTCOMPOSER_HANDLEGENERATE_END ===
  ...
  const options = (
    <>
      ...
      <button type="button" onClick={handleGenerate} disabled={generating} style={primaryBtn}>
  ...
  const exportBoxEl = result && result.ok && (
    <div style={exportBox}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
        <button type="button" onClick={() => void handleChooseLocation()} disabled={exporting} style={secondaryBtn}>
          다른 위치에 저장…
        </button>
        {/* inline 모드는 푸터가 없으므로 '파일 열기'를 여기에 둔다. */}
        {inline && (
          <button
            type="button"
            onClick={() => {
  ...
  // === ANCHOR: REPORTCOMPOSER_PREVIEW_START ===
  const previewEl = (fillHeight: boolean) => {
    if (result && result.ok && result.kind === "html") {
      return (
        <div style={fillHeight ? { display: "flex", flexDirection: "column", height: "100%" } : { marginTop: 12 }}>
          <iframe
            title="보고서 미리보기"
            srcDoc={result.html}
            sandbox=""
            style={fillHeight ? { flex: 1, width: "100%", border: "1px solid #ddd", background: "#fff" } : { width: "100%", height: 420, border: "1px solid #ddd" }}
          />
    ...
    if (result && result.ok && result.kind === "file") {
      // PDF 는 pdf.js 로 인앱 렌더(페이지 나눔·번호까지 최종 결과 그대로).
      // Word·PPT 는 인앱 렌더가 어려워 경로만 안내하고 '파일 열기'로 처리한다.
      if (/\.pdf$/i.test(result.path)) {
        const pdfPath = result.path;
        return fillHeight ? (
          <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
            <div style={{ flex: 1, minHeight: 0 }}>
              <PdfPreview cwd={cwd} path={pdfPath} />
            </div>
            <p style={{ fontSize: 11, color: "#999", marginTop: 8 }}>내부 사본: {pdfPath}</p>
          </div>
        ) : (
      ...
      return <p style={{ fontSize: 11, color: "#999", marginTop: 12 }}>내부 사본: {result.path}</p>;
    }
    if (fillHeight) {
      return (
        <div
  ...
  // === ANCHOR: REPORTCOMPOSER_PREVIEW_END ===
  ...
  if (inline) {
    return (
      <div style={{ display: "flex", height: "100%", gap: 16, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>{previewEl(true)}</div>
      </div>
    );
  }
