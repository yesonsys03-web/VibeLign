...
export const JOURNEY_STEPS: JourneyStep[] = [
  ...
  },
  {
    ...
    howto: [
      ...
      "⑤ (선택) 확정한 기획안에서 🎨 디자인 미리보기를 누르면, 코딩 전에 디자인을 먼저 정할 수 있어요 — 고른 디자인으로 작업방이 시작돼요",
    ],
    targetPage: "planning",
    goLabel: "기획하러 가기 →",
    cardStage: "planning",
  },
  {
    ...
    icon: "3️⃣",
    label: "체크포인트 저장",
    shortAction: "작업방을 쓰면 자동 저장 — 외부 도구로 직접 작업할 때만 수동으로 저장하세요",
    howto: [
      "💡 작업방(4단계)을 쓰면 실행 직전 체크포인트가 자동 저장돼요 — 이 단계는 따로 안 해도 돼요.",
      "외부 도구(터미널)로 직접 작업할 때만 수동으로:",
      "① 유지보수 → 백업 탭으로 가요",
      "② \"체크포인트 저장\" 버튼을 눌러요",
  ...
  },
  {
    ...
    icon: "5️⃣",
    label: "결과 검증",
    // 검증의 두 축: 안전(약속 범위만 고쳤나=상태 확인)·작동(진짜 켜지나=실행해보기).
    // 사용자 노출 카피에 "guard" 영문 노출 금지 — UI 버튼 라벨('상태 확인')과 동일 어휘 사용 (북극성 점검 #3)
    shortAction: "'상태 확인'으로 약속 범위를(안전), '실행해보기'로 진짜 작동을(작동) 확인하세요",
    howto: [
      "💡 작업방에서 실행했다면 안전 검사(상태 확인)는 이미 자동으로 끝났어요 — 결과가 작업방 완료 화면에 떠요.",
      "① (외부 도구로 작업했다면) 홈에서 '상태 확인' — AI가 약속한 범위만 고쳤는지 검사해줘요(안전)",
      "② 개발 → 실행해보기에서 ▶ 실행 — 진짜 켜지는지 미리보기로 직접 써봐요(작동)",
      "③ 문제가 있으면 진단으로 안내돼요 — 고치는 길을 알려줘요",
      "④ 결과가 마음에 안 들면? 백업 탭에서 '되돌리기'로 3️⃣ 시점으로 돌아가도 돼요",
    ],
